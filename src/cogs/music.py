...
class Music(commands.Cog):
    def __init__(self, bot):
        ...
        if not hasattr(bot, 'lavalink'):  # This ensures the client isn't overwritten during cog reloads.
            ...
            bot.lavalink.add_node(os.getenv("LAVALINK_HOST"), 2333, os.getenv("LAVALINK_PASSWORD"), 'in', 'default-node')  # Host, Port, Password, Region, Name
    ...
    async def now(self, ctx):
        ...
        if player.current:
            ...
            em.set_footer(text=f"Requested by: {requester}", icon_url=requester.avatar.url)
